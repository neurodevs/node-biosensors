...
import { MUSE_2_CHAR_UUIDS } from '../../../impl/devices/Muse2.js'
...
export default class Muse2Test extends AbstractDeviceControllerBleTest {
    ...
    private static readonly charCallbacks = Object.entries(
        MUSE_2_CHAR_UUIDS
    ).map(([name, uuid]) => {
        return {
            charUuid: uuid,
    ...
    protected static async startStreamingWritesMuse2CommandsToControlChar() {
        ...
        assert.isEqualDeep(
            ...
            [
                this.generateCmd('h'),
                this.generateCmd('p51'),
                this.generateCmd('s'),
                this.generateCmd('d'),
            ],
            'Should write the Muse 2 preset (p51) start commands to control char!'
        )
    }
    ...
    private static async MuseDeviceController(
        ...
        return (await MuseDeviceController.Create('Muse 2', {
